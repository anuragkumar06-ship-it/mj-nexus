...
function template(b: any): string {
  ...
  if (b.type === "Appreciation") {
    return `It is my pleasure to recognize ${b.name} for outstanding contributions to the ${b.team} team at MJ Marketing Consultancy. ${first} consistently exceeded expectations, maintaining a ${b.performance}% performance score and a ${b.reliability}% reliability index. ${first} brought energy, ownership, and a growth mindset to every project, and is a genuine asset to any team.`;
  }
  return `To whom it may concern,\n\nI am delighted to recommend ${b.name}, who completed a ${b.role} internship with the ${b.team} team at MJ Marketing Consultancy. Over the program, ${first} achieved a ${b.performance}% performance score, a ${b.reliability}% reliability index, and a ${b.growth}% growth index — reflecting strong execution, dependability, and rapid development.\n\n${first} is proactive, collaborative, and consistently delivers high-quality work. I recommend ${first} without reservation for any future role.`;
}
...
export async function POST(req: Request) {
  ...
  try {
    const prompt =
      ...
      `a ${b.role} intern on the ${b.team} team at MJ Marketing Consultancy. ` +
      `Metrics — performance ${b.performance}%, reliability ${b.reliability}%, growth ${b.growth}%. ` +
      `Be specific, sincere, and concise (max 140 words). Sign off as "M. Joshi, Founder, MJ Marketing Consultancy".`;
    const letter = await chat(
      [
        { role: "system", content: "You write concise, professional HR letters for MJ Marketing Consultancy." },
