...
import { FileText, Sparkles, Zap, Star, ScanSearch, Brain } from "lucide-react";
...
export default function AiEnginePage() {
  ...
  const avgFit = parsed ? Math.round(candidates.reduce((s, c) => s + (c.fitScore ?? 0), 0) / parsed) : 0;
  const shortlisted = candidates.filter((c) => (c.fitScore ?? 0) >= 80).length;
  const strong = candidates.filter((c) => (c.fitScore ?? 0) >= 88).length;

  const kpis = [
    { label: "Candidates Scored", value: parsed, icon: <FileText className="h-5 w-5" /> },
    { label: "Avg Fit Score", value: avgFit, icon: <Sparkles className="h-5 w-5" /> },
    { label: "Auto-Shortlisted (≥80)", value: shortlisted, icon: <Zap className="h-5 w-5" /> },
    { label: "Strong Hires (≥88)", value: strong, icon: <Star className="h-5 w-5" /> },
  ];
        ...
        <Reveal delay={0.05}>
          <Card>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {criteria.map((c, i) => (
                <div key={c.label} className="rounded-2xl border border-navy/5 bg-offwhite/60 p-4">
                  <div className="mb-2 grid h-8 w-8 place-items-center rounded-xl bg-gradient-brand text-xs font-bold text-white">{i + 1}</div>
